#!/usr/bin/env node
// QVAC Fake Board Meeting Excuse Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29422;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/meetingexcuse") {
      try {
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Board Meeting Excuse Generator GUI ready at http://localhost:${PORT}`);
  });
